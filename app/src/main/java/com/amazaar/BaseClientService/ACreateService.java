...
public class ACreateService<Pb extends GeneratedMessageV3> extends AsyncTask<Pb, Void, Pb> {

    private Class<Pb> m_clazz;
    private ProtoSerilizerAndDeserilizer m_serilizer;
    private UrlPathProvider.UrlPathEnum m_pathEnum;

    public ACreateService(Class<Pb> clazz,UrlPathProvider.UrlPathEnum pathEnum) {
        m_clazz = clazz;
    ...
    protected Pb doInBackground(Pb... workerPbs) {
        ...
        HttpCaller caller = new HttpCaller(RequestMethodEnum.POST, RequestContentTypeEnum.CONTENT_TYPE_JSON, urlManeger.getServerUrl(m_pathEnum), m_serilizer.getJsonObject(workerPbs[0]));
        try {
